...
df = pd.read_csv("data/genotype.csv", index_col=0)
...
# On ajoute la colonne chr juste en fonction de l'indexe - reset_index pour mettre l'index (les SNPs) en colonne 
df = df.reset_index().join(df_chr["Chr"])
df = df.rename(columns = {"index" : "SNP"})
df.columns
...
df_export.to_csv("data/df_filtered.csv", index=True)
