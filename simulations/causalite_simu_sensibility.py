...
@st.cache_data
def load_data():
    df = pd.read_csv("data/df_filtered.csv", index_col=0)
...
if page == "Exploration":
    ...
    if st.button("Manhattan Plot like graph"):
        # Alors pour générer le manhattan plot, c'est un peu lourd pour quelque chose d'assez simple :
    ...
    if st.button("Comparer les méthodes"):
        ...
        for i in range(n_simu):
            ...
            def check_sig(val_series, snp_series):
                # Avec la librairie scipy, on aurait pu utiliser penguins ou pingouins ? 
    ...
    with col_s2:
        ...
        fix_ate = st.slider("Fixer ATE", 0.0, 3.0, 0.8) if param_to_vary != "ATE (2 vs 0)" else None
    ...
    if st.button("Lancer l'analyse"):
        ...
        for idx, val in enumerate(range_values):
            ...
            for _ in range(n_simu):
                ...
                nmf_v = NMF(n_components=1, init='nndsvd', max_iter=500).fit_transform(X - X.min().min() + 0.01).flatten()
...
# Critiques sur le code : des redondances, comme la selection des paramètres qui reviennent à chaque fois
# Ou alors le filtrage des 0 et 2 que je devrais faire au début ``
# Dans la partie analyse de puissance, la NMF n'a pas assez d'iterations pour réellment converger 
# Dans cette même partie j'ai ce warning : 2026-02-13 09:54:54.985 Please replace `use_container_width` with `width`.
# Utilisation de pandas - migration progressive vers polars ?
